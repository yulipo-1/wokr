...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'solveQuestion') {
    solveQuestion(request.question, request.options, request.apiKey, sender)
...
async function solveQuestion(questionText, options, apiKey, sender) {
  ...
  // Get the referer from the sender's tab URL or use a default
  const referer = sender?.tab?.url ? new URL(sender.tab.url).origin : 'https://test-site.com';
